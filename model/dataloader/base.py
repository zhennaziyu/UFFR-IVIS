...
class BaseDataset(Dataset):
    ...
    def __init__(self, setname, unsupervised, args, augment='none', clean=True):
    ...
    def get_data(self, setname):
        ...
        if self.use_im_cache:
            ...
            # data, label = self.parse_csv(csv_path)
            # plit_path = '/home/zhengna/PsCo-main/data/Food2K/largeFood2K'
            plit_path = self.split_path
            if setname == 'train':
                data = json.load(open(osp.join(plit_path, 'dis_train.json'), 'r'))
                label = json.load(open(osp.join(plit_path, 'dis_train_label.json'), 'r'))

            elif setname == 'val':
                data = json.load(open(osp.join(plit_path, 'MiniTest1.json'), 'r'))
                label = json.load(open(osp.join(plit_path, 'MiniTest1_label.json'), 'r'))
            else:
                data = json.load(open(osp.join(plit_path, 'MiniTest1.json'), 'r'))
                label = json.load(open(osp.join(plit_path, 'MiniTest1_label.json'), 'r'))
            return data, label
    ...
    def get_transform(self, args, augment, image_size, setname):
        if setname == 'train':
            if augment == 'AMDIM':
                ...
                return [transforms.Compose(self.strong_trans), transforms.Compose(self.weak_trans)]
            elif augment == 'test':
    ...
    def __getitem__(self, i):
        ...
        data = osp.join(self.args.data_root, data)
        label = int(label)
        ...
        if self.unsupervised:
            strong_trans, weak_trans = self.transform
            data_s = strong_trans(data)
            data_w = weak_trans(data)

            return data_s, data_w, label,i
            # return data_aug, data_w, label
        else:
