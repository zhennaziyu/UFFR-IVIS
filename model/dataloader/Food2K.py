...
class Food_2K(BaseDataset):
    """ Usage:
    """

    def __init__(self, setname, unsupervised, args, augment='none', clean=True):
        self.IMAGE_PATH1 = search_dir([args.data_root])
        # self.IMAGE_PATH1 = search_dir([osp.join(args.data_root, di, 'images') for di in data_dirs])
        self.SPLIT_PATH = osp.join(ROOT_PATH, 'data/Food2K/split')
        self.SPLIT_PATH_LARGE = osp.join(ROOT_PATH, 'data/Food2K/largeFood2K')
