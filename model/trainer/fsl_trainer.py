    ...
    def train(self):
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            for batch in self.train_loader:
                data_s, data_w, label, index = batch
                # print(data_s.shape)
                self.train_step += 1

                self.optimizer.zero_grad()
                if torch.cuda.is_available():
                    data_s, data_w, label, index = [_.cuda() for _ in batch]
                else:
                    data_s, data_w, label, index = batch[0], batch[1], batch[2], batch[3]

                data_tm = time.time()
                self.dt.add(data_tm - start_tm)

                loss = self.model([data_s, data_w], epoch=epoch, label_gt=label, index = index)
                loss.backward()
                self.optimizer.step()
    ...
    def obtain_feature(self,pth):
        ...
        with torch.no_grad():
            for batch in tqdm(self.train_loader):
                data_s, data_w, label = batch
                # print(data_s.shape)

                if torch.cuda.is_available():
                    data_s, data_w, _ = [_.cuda() for _ in batch]
                else:
                    data_s, data_w, _ = batch[0], batch[1], batch[2]

                data_tm = time.time()
                # x, get_feature=False, epoch=0, **kwargs
                feat, neighbor = self.model([data_s, data_w], True)
                print(F.normalize(feat, dim=-1, p=2))
                if feats == None:
                    feats = feat
                    neighs = neighbor
                else:
                    feats = torch.cat([feats, feat], dim=0)
                    neighs = torch.cat([neighs, neighbor], dim=0)
                # print(feats.shape)
                # print(feats.shape)
            # feats = feats.to('cpu')
            # torch.save(feats,'feats_res')
            # neighs = neighs.to('cpu')
            # torch.save(neighs, 'neighs, res')
